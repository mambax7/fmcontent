...
function fmcontent_list_show($options)
{
    ...
    $options0                 = $options[0];
    $content_infos ['topics'] = $topicHandler->getAll();
    $contents                 = $pageHandler->getContentBlockList($forMods, $content_infos, $options);
...
function fmcontent_list_edit($options)
{
    ...
    $form .= '<option value="0" ' . (false === array_search(0, $options, true) ? '' : 'selected') . '>' . _MI_FMCONTENT_ALLMENUS . "</option>\n";
    foreach (array_keys($topic_arr) as $i) {
        $form .= '<option value="' . $topic_arr[$i]->getVar('topic_id') . '" ' . (false === array_search($topic_arr[$i]->getVar('topic_id'), $options, true) ? '' : 'selected') . '>' . $topic_arr[$i]->getVar('topic_title') . "</option>\n";
    }
    $form .= "</select>\n";
