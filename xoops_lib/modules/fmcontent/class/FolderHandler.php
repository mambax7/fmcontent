 ...
 * @lastmodified $Date: 2011-11-06 04:41:43 -0500 (Sun, 06 Nov 2011) $
...
class FolderHandler
{
    ...
    public function read($sort = true, $exceptions = false)
    {
        ...
        if (false !== $dir) {
            while (false !== ($n = readdir($dir))) {
                ...
                } elseif ((!preg_match('/^\\.+$/', $n) && false === $exceptions)
                          || (true === $exceptions
                              && !preg_match('/^\\.(.*)$/', $n))) {
                    $item = $n;
                }
    ...
    /**
     * Private helper public function for findRecursive.
     *
    ...
    public function create($pathname, $mode = false)
    {
        ...
        if ($this->create($nextPathname, $mode)) {
            if (!file_exists($pathname)) {
                if (mkdir($pathname, intval($mode, 8)) || is_dir($pathname)) {
    ...
    public function delete($path)
    {
        ...
        if (is_dir($path)) {
            ...
            $normal_files = glob($path . '*', GLOB_NOSORT);
            $hidden_files = glob($path . '\.?*', GLOB_NOSORT);
    ...
    public function copy($options = [])
    {
        ...
        if (!is_dir($toDir)) {
            if (!mkdir($toDir, $mode) && !is_dir($toDir)) {
    ...
    public function realpath($path)
    {
        ...
        if ('' != $path > 1 && '/' === $path[mb_strlen($path) - 1]) {
            $newpath .= '/';
        }
