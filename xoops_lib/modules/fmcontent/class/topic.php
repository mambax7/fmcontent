...
class Topic extends \XoopsObject
{
    ...
    {
        $form = new \XoopsThemeForm(_FMCONTENT_FORM_TOPIC, 'topic', 'backend.php', 'post', true);
        ...
        $form->addElement(new \XoopsFormLabel(_FMCONTENT_TOPIC_PARENT, ob_get_clean()));
        ...
        // Image
        $topic_img                = $this->getVar('topic_img') ?: 'blank.gif';
        ...
        global $xoopsModule;
        if (!$this->isNew()) {
            $groups_ids_view                 = $grouppermHandler->getGroupIds('fmcontent_access', $this->getVar('topic_id'), $xoopsModule->getVar('mid'));
            $groups_ids_submit               = $grouppermHandler->getGroupIds('fmcontent_submit', $this->getVar('topic_id'), $xoopsModule->getVar('mid'));
